...
# Confirmed against real SharpAPI odds-row dumps on 2026-08-27 (not just the
# /leagues catalog, which lists differently-named/hyphenated variants like
# "usa_-_nfl_preseason" that don't actually match what shows up in the
# "league" field on real odds rows). Preseason games come back tagged
# exactly league="nfl_preseason", separate from league="nfl" -- a request
# scoped to just "nfl" will never see them, no matter how correct its
# pagination is, since it's a genuinely separate league bucket on
# SharpAPI's side, not a season_type flag within "nfl".
NFL_PRESEASON_LEAGUE = "nfl_preseason"
NFL_REGULAR_LEAGUE = "nfl"


def nfl_leagues_for_build(is_preseason):
    """Return the league id(s) to pass as `league=` to fetch_all_odds() for
    an NFL build, given whether this build is currently in the preseason
    window.

    During preseason: fetches both "nfl" and "nfl_preseason" together (as
    a tuple), since fetch_all_odds() now supports merging multiple leagues
    into one row list. Belt-and-suspenders -- if any regular-season games
    are also on the board that early (rare, but cheap to cover), they're
    not missed either.

    Outside preseason: fetches "nfl" alone, so we don't spend a request
    per build asking a league that reliably has nothing posted once the
    preseason window has closed.

    `is_preseason` should come from whatever season_type detection the
    build script already does against ESPN's scoreboard (the same signal
    used to label weeks "P1"/"P2" vs. "Regular Season Week N") -- this
    function doesn't do that detection itself, since it lives in
    build_nfl_dashboard.py, not here.
    """
    if is_preseason:
        return (NFL_REGULAR_LEAGUE, NFL_PRESEASON_LEAGUE)
    return NFL_REGULAR_LEAGUE
