...
def _extract_season_type(payload):
    """Pull the numeric season type (1=preseason/spring training,
    2=regular, 3=postseason) out of an ESPN scoreboard response, same
    extraction build_nfl_dashboard.py's own _extract_season_type() uses.
    Tries the top-level `season.type` field first, then falls back to
    `leagues[0].season.type`. Returns None if neither is present (caller
    decides the fallback -- see is_preseason_day() below)."""
...
def is_preseason_day(scoreboard):
    """Whether a fetched day's scoreboard is spring training (exhibition)
    games, so build_day() can tag the day and the front end can show a
    "Spring Training" badge instead of treating them identically to real
    ...
    spring training would be worse than the reverse, which just leaves
    an exhibition day unbadged."""
...
def build_day(day, sharp_key, gemini_key=None, previous_odds_by_id=None,
    ...
    log(f"  {len(events)} games" + (" (spring training)" if is_preseason else ""))
